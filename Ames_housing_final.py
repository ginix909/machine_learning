...
может быть выкинуть из фрейма значения более 400 или для начал 600 тыс долларов. И давай как исследуем актуальные значения
продажи, имеют ли они нормальное распределение. для этого проверим признаки нормального распределения:
унимодально
симметрично
отклонения наблюдений от среднего подчиняются вероятностному закону. правилу 3 сигм.
Вычислить это можно с помощью Центральной предельной теоремы.
Если гипотеза верна, распределение нормальное. то я удалю значения более 2 сигм например. то есть оставлю 95% распределений
Это может и неправильно. но возможно я избавлюсь от выбросов. И увеличу точность предскзаний..'''

'''
Какие метрики еще можно посчитать?

!!! Построй график предсказания и фактические значения! возьми по k=0 факт и предсказанные и посмтори на них.
может быть возьми k=10 фактические и предсказанные..

Как там найти лучший классификатор? Best estimator.
Что можно настроить чтобы попробовать улучшить результат: меньше колонок по корреляции с целевым признаком, выше порог
по пропущеннм значеням, выше порог по количеству вариантов категориальных переменных.
Как понять не переобучилась ли модель? 
Пройди по всему алгоритму отбора и инжиниринга данных и посмотри где можно уменьшить.
